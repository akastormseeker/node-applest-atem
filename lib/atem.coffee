...
class ATEM
  ...
  _setStatus: (name, buffer) ->
    ...
    switch name
      ...
      when 'TMxP' # Transition Mix Parameters
        me = buffer[0]
        @state.video.ME[me].transitionParams[0].rate = buffer[1]

      when 'TDpP' # Transition Dip Parameters
        me = buffer[0]
        @state.video.ME[me].transitionParams[1].rate = buffer[1]
        @state.video.ME[me].transitionParams[1].input = @_parseNumber(buffer[2..3])

      when 'TWpP' # Transition Wipe Parameters
        me = buffer[0]
        @state.video.ME[me].transitionParams[2].rate = buffer[1]
        @state.video.ME[me].transitionParams[2].pattern = buffer[2]
        @state.video.ME[me].transitionParams[2].unknown3 = buffer[3]
        @state.video.ME[me].transitionParams[2].width = @_parseNumber(buffer[4..5])
        @state.video.ME[me].transitionParams[2].fillSource = @_parseNumber(buffer[6..7])
        @state.video.ME[me].transitionParams[2].symmetry = @_parseNumber(buffer[8..9])
        @state.video.ME[me].transitionParams[2].softness = @_parseNumber(buffer[10..11])
        @state.video.ME[me].transitionParams[2].positionX = @_parseNumber(buffer[12..13])
        @state.video.ME[me].transitionParams[2].positionY = @_parseNumber(buffer[14..15])
        @state.video.ME[me].transitionParams[2].reverse = buffer[16] == 0x01
        @state.video.ME[me].transitionParams[2].flipFlop = buffer[17] == 0x01
        @state.video.ME[me].transitionParams[2].unknown18 = buffer[18]

      when 'TDvP' # Transition DVE Parameters
        me = buffer[0]
        @state.video.ME[me].transitionParams[3].rate = buffer[1]
        @state.video.ME[me].transitionParams[3].unknown2 = buffer[2]
        @state.video.ME[me].transitionParams[3].style = buffer[3]
        @state.video.ME[me].transitionParams[3].fillSource = @_parseNumber(buffer[4..5])
        @state.video.ME[me].transitionParams[3].keySource = @_parseNumber(buffer[6..7])
        @state.video.ME[me].transitionParams[3].enableKey = buffer[8] == 0x01
        @state.video.ME[me].transitionParams[3].preMultiplied = buffer[9] == 0x01
        @state.video.ME[me].transitionParams[3].clip = @_parseNumber(buffer[10..11])
        @state.video.ME[me].transitionParams[3].gain = @_parseNumber(buffer[12..13])
        @state.video.ME[me].transitionParams[3].invert = buffer[14] == 0x01
        @state.video.ME[me].transitionParams[3].reverse = buffer[15] == 0x01
        @state.video.ME[me].transitionParams[3].flipFlop = buffer[16] == 0x01
        @state.video.ME[me].transitionParams[3].unknown17 = buffer[17]
        @state.video.ME[me].transitionParams[3].unknown18 = buffer[18]
        @state.video.ME[me].transitionParams[3].unknown19 = buffer[19]

      when 'TStP' # Transition Stinger Parameters
        me = buffer[0]
        @state.video.ME[me].transitionParams[4].source = buffer[1]
        @state.video.ME[me].transitionParams[4].preMultiplied = buffer[2] == 0x01
        @state.video.ME[me].transitionParams[4].clip = @_parseNumber(buffer[4..5])
        @state.video.ME[me].transitionParams[4].gain = @_parseNumber(buffer[6..7])
        @state.video.ME[me].transitionParams[4].invertKey = buffer[8] == 0x01
        @state.video.ME[me].transitionParams[4].unknown9 = buffer[9]
        @state.video.ME[me].transitionParams[4].preRoll = @_parseNumber(buffer[10..11])
        @state.video.ME[me].transitionParams[4].clipDuration = @_parseNumber(buffer[12..13])
        @state.video.ME[me].transitionParams[4].triggerPoint = @_parseNumber(buffer[14..15])
        @state.video.ME[me].transitionParams[4].mixRate = @_parseNumber(buffer[16..17])
        @state.video.ME[me].transitionParams[4].unknown18 = buffer[18]
  ...
  setTransitionDipInput: (input, me = 0) ->
    @_sendCommand('CTDp', [0x02, me, 0x00, input >> 8, input & 0xFF])



  setTransitionWipeRate: (rate, me = 0) ->
    @_sendCommand('CTWp', [0x00, 0x01, me, rate, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionWipePattern: (pattern, me = 0) ->
    @_sendCommand('CTWp', [0x00, 0x02, me, 0x00, pattern, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionWipeWidth: (width, me = 0) ->
    @_sendCommand('CTWp', [0x00, 0x04, me, 0x00, 0x00, 0x00, width/256, width%256, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionWipeFillSource: (source, me = 0) ->
    @_sendCommand('CTWp', [0x00, 0x08, me, 0x00, 0x00, 0x00, 0x00, 0x00, source >> 8, source & 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionWipeSymmetry: (symmetry, me = 0) ->
    @_sendCommand('CTWp', [0x00, 0x10, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, symmetry/256, symmetry%256, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
    
  setTransitionWipeSoftness: (softness, me = 0) ->
    @_sendCommand('CTWp', [0x00, 0x20, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, softness/256, softness%256, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
  
  setTransitionWipePositionX: (positionx, me = 0) ->
    @_sendCommand('CTWp', [0x00, 0x40, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, positionx/256, positionx%256, 0x00, 0x00, 0x00, 0x00])

  setTransitionWipePositionY: (positiony, me = 0) ->
    @_sendCommand('CTWp', [0x00, 0x80, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, positiony/256, positiony%256, 0x00, 0x00])

  setTransitionWipeReverse: (state, me = 0) ->
    @_sendCommand('CTWp', [0x01, 0x00, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, state ? 0x01 : 0x00, 0x00])

  setTransitionWipeFlipFlop: (state, me = 0) ->
    @_sendCommand('CTWp', [0x02, 0x00, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, state ? 0x01 : 0x00])



  setTransitionDVERate: (rate, me = 0) ->
    @_sendCommand('CTDv', [0x00, 0x01, me, rate, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionDVEStyle: (style, me = 0) ->
    @_sendCommand('CTDv', [0x00, 0x04, me, 0x00, 0x00, style, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionDVEFillSource: (source, me = 0) ->
    @_sendCommand('CTDv', [0x00, 0x08, me, 0x00, 0x00, 0x00, source >> 8, source & 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionDVEKeySource: (source, me = 0) ->
    @_sendCommand('CTDv', [0x00, 0x10, me, 0x00, 0x00, 0x00, 0x00, 0x00, source >> 8, source & 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionDVEEnableKey: (enable, me = 0) ->
    @_sendCommand('CTDv', [0x00, 0x20, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, enable ? 0x01 : 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionDVEPreMultiplied: (enable, me = 0) ->
    @_sendCommand('CTDv', [0x00, 0x40, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, enable ? 0x01 : 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionDVEClip: (clip, me = 0) ->
    @_sendCommand('CTDv', [0x00, 0x80, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, clip/256, clip%256, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionDVEGain: (gain, me = 0) ->
    @_sendCommand('CTDv', [0x01, 0x00, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, gain/256, gain%256, 0x00, 0x00, 0x00, 0x00])

  setTransitionDVEInvertKey: (enable, me = 0) ->
    @_sendCommand('CTDv', [0x02, 0x00, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, enable ? 0x01 : 0x00, 0x00, 0x00, 0x00])

  setTransitionDVEReverse: (enable, me = 0) ->
    @_sendCommand('CTDv', [0x04, 0x00, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, enable ? 0x01 : 0x00, 0x00, 0x00])

  setTransitionDVEFlipFlop: (enable, me = 0) ->
    @_sendCommand('CTDv', [0x08, 0x00, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, enable ? 0x01 : 0x00, 0x00])



  setTransitionStingerSource: (source, me = 0) ->
    @_sendCommand('CTSt', [0x00, 0x01, me, source, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionStingerPreMultiplied: (enabled, me = 0) ->
    @_sendCommand('CTSt', [0x00, 0x02, me, 0x00, enabled ? 0x01 : 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionStingerClip: (clip, me = 0) ->
    @_sendCommand('CTSt', [0x00, 0x04, me, 0x00, 0x00, 0x00, clip/256, clip%256, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionStingerGain: (gain, me = 0) ->
    @_sendCommand('CTSt', [0x00, 0x08, me, 0x00, 0x00, 0x00, 0x00, 0x00, gain/256, gain%256, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionStingerInvertKey: (enabled, me = 0) ->
    @_sendCommand('CTSt', [0x00, 0x10, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, enabled ? 0x01 : 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

  setTransitionStingerPreRoll: (frames, me = 0) ->
    @_sendCommand('CTSt', [0x00, 0x20, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, frames >> 8, frames & 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
  
  setTransitionStingerClipDuration: (frames, me = 0) ->
    @_sendCommand('CTSt', [0x00, 0x40, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, frames >> 8, frames & 0xFF, 0x00, 0x00, 0x00, 0x00])

  setTransitionStingerTriggerPoint: (frames, me = 0) ->
    @_sendCommand('CTSt', [0x00, 0x80, me, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, frames >> 8, frames & 0xFF, 0x00, 0x00])

  setTransitionStingerMixRate: (frames, me = 0) ->
